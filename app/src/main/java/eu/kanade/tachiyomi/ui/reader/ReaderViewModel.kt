...
import eu.kanade.tachiyomi.ui.reader.translation.OcrManager
import eu.kanade.tachiyomi.ui.reader.translation.LanguageDetector
import eu.kanade.tachiyomi.ui.reader.translation.TranslatorManager
...
class ReaderViewModel @JvmOverloads constructor(
    ...
    private val languageDetector by lazy { LanguageDetector() }
    private val translatorManager by lazy { TranslatorManager() }
...
//                // KMK <--
    ...
    fun onPageSelected(page: ReaderPage, currentPageText: String /* SY --> */, hasExtraPage: Boolean /* SY <-- */) {
        // KMK -->
        mutableState.update { it.copy(translationResult = null, isTranslating = false) }
        if (state.value.isAutoTranslating) {
            viewModelScope.launch {
                kotlinx.coroutines.delay(1000) // Wait for settle
                if (state.value.isAutoTranslating && !state.value.isTranslating) {
                    mutableState.update { it.copy(isTranslating = true, translationResult = null) }
                    eventChannel.send(Event.CaptureScreen)
                }
            }
        }
        // KMK <--
    ...
    fun translatePage() {
        ...
        mutableState.update { it.copy(translationResult = null) }
        viewModelScope.launch {
            kotlinx.coroutines.delay(3000)
            if (state.value.isTranslating) return@launch
    ...
    fun toggleAutoTranslation() {
        if (state.value.isAutoTranslating) {
            // If already on, manual click acts as refresh
            mutableState.update { it.copy(translationResult = null, isTranslating = true) }
            viewModelScope.launch {
                kotlinx.coroutines.delay(2000)
                eventChannel.send(Event.CaptureScreen)
            }
        } else {
            mutableState.update { it.copy(isAutoTranslating = true, isTranslating = true, translationResult = null) }
            viewModelScope.launch {
                kotlinx.coroutines.delay(2000)
                eventChannel.send(Event.CaptureScreen)
            }
    ...
    fun disableAutoTranslation() {
        mutableState.update { it.copy(isAutoTranslating = false, isTranslating = false, translationResult = null) }
    }
    ...
    fun onViewSettled() {
        if (state.value.isAutoTranslating && !state.value.isTranslating && state.value.translationResult == null) {
            viewModelScope.launch {
                kotlinx.coroutines.delay(2000) // Settle delay
                if (state.value.isAutoTranslating && !state.value.isTranslating && state.value.translationResult == null) {
                    mutableState.update { it.copy(isTranslating = true) }
                    eventChannel.send(Event.CaptureScreen)
                }
    ...
    fun onScreenCaptured(bitmap: Bitmap) {
        viewModelScope.launchIO {
            try {
                val sourceLangCode = readerPreferences.translationSourceLang().get()
                val targetLangCode = readerPreferences.translationTargetLang().get()

                // 1. OCR
                val visionText = ocrManager.processImageInternal(bitmap, sourceLangCode) ?: throw Exception("OCR failed")

                // 2. Language Detection
                val detectedSourceLang = if (sourceLangCode == "auto") {
                    languageDetector.detectLanguageSync(visionText.text)
                } else {
                    sourceLangCode
                }

                // Check and download models if needed
                checkAndDownloadModels(detectedSourceLang, targetLangCode) { success ->
                    if (!success) {
                        mutableState.update { it.copy(isTranslating = false, isDownloadingModel = false) }
                        return@checkAndDownloadModels
                    }

                    // 3. Translation
                    translatorManager.translateTexts(visionText, sourceLanguage = detectedSourceLang, targetLanguage = targetLangCode) { translatedText ->
                        val blocks = mutableListOf<eu.kanade.tachiyomi.ui.reader.translation.TranslatedBlock>()
                        visionText.textBlocks.forEachIndexed { index, textBlock ->
                            if (index < translatedText.size) {
                                blocks.add(eu.kanade.tachiyomi.ui.reader.translation.TranslatedBlock(translatedText[index], textBlock.boundingBox!!))
                            }
                        }
                        ...
                        mutableState.update {
                            if (it.isTranslating) {
                                it.copy(
                                    ...
                                    translationResult = eu.kanade.tachiyomi.ui.reader.translation.TranslationResult(blocks, detectedSourceLang, targetLangCode)
                                )
                            } else {
                                it
                            }
    ...
    private fun checkAndDownloadModels(sourceLang: String, targetLang: String, onResult: (Boolean) -> Unit) {
        translatorManager.isModelDownloaded(sourceLang) { sourceDownloaded ->
            if (sourceDownloaded) {
                translatorManager.isModelDownloaded(targetLang) { targetDownloaded ->
                    if (targetDownloaded) {
                        onResult(true)
                    } else {
                        downloadModel(targetLang, onResult)
                    }
                }
            } else {
                downloadModel(sourceLang) { sourceSuccess ->
                    if (sourceSuccess) {
                        translatorManager.isModelDownloaded(targetLang) { targetDownloaded ->
                            if (targetDownloaded) {
                                onResult(true)
    ...
    private fun downloadModel(langCode: String, onResult: (Boolean) -> Unit) {
        mutableState.update { it.copy(isDownloadingModel = true) }
        translatorManager.downloadModel(langCode) { success ->
            onResult(success)
        }
    }
    // SY <--
    ...
    data class State(
        ...
        val translationResult: eu.kanade.tachiyomi.ui.reader.translation.TranslationResult? = null,
        // SY <--
    ) {
    ...
    sealed interface Event {
        ...
        data object CaptureScreen : Event
    }
}
